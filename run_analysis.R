...
# The "UCI HAR" folder must reside in the working directory, with all the data set files as provided in 
#     the .zip file (./UCI HAR/*). 
# the package "reshape2" must be installed.

# Output: The function writes the resulting tidy data set in the file "./tidyDataSet.txt", and returns the tidy data set 
#         as a data frame.
...
# The 9 signals files in the ./test/Inertial Signals and ./training/Inertial Signals folders were not used as they are 
# not required.
...
run_analysis <- function () {  
  ...
  #   - Get the sequence of activities observed for test and training. Each record represents the activity for one 
  #     sliding window. 
  ...
  #       Note: the merge function changes the order of the records. To remember the order, the row.names were added as 
  #       a column before merging.
  ...
  labeledActivities <- merge(activityLabels, activities, by = "V1", all.y=TRUE, sort=FALSE)   # merge activities with 
                                                                                              # activity labels 
  labeledActivities <- labeledActivities[order(labeledActivities$rn),]                        # resort records based on the 
                                                                                              # original row.names saved before
  
  rownames(labeledActivities) <- labeledActivities$rn                                         # reassign new row.names now that 
                                                                                              # order correct
  ...
  #   - Append training and test rows together using rbind
  #   - Reduce the featuresVector in the appended data set to have only the mean() and std() measurements. 
  #     Reduced from 561 to 66 measurements. 
  ...
  #   - dcast the resulting dataset to put back the measurements as columns and calculate at the ame time the mean by 
  #     activityType, subject, and measurement
